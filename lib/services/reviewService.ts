...
const reviewService = {
  ...
  // 특정 도서의 리뷰 목록
  getBookReviews: async (bookId: number, params: { page?: number; size?: number; sort?: string } = {}) => {
  ...
  // 내 리뷰 목록
  getMyReviews: async (params: { page?: number; size?: number; sort?: string } = {}) => {
    const { page = 0, size = 10, sort = 'createdAt,desc' } = params;
  ...
  // 리뷰 투표
  voteReview: async (reviewId: number, voteType: string) => {
    return await api.post(`/reviews/${reviewId}/vote`, { voteType });
  },

  // 리뷰 투표 취소
  cancelVote: async (reviewId: number) => {
